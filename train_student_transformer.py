...
Architecture (trainable part, ~12M params):
  Frozen Qwen2.5-Coder-3B (all params fixed, forward hook captures layer-18
  token-level hidden states)  →  SigPredictor:
    Linear(2048 → 512)
    2× TransformerEncoderLayer(512, nhead=8, ffn=2048, dropout=0.1)
    mean-pool over non-padding tokens
    Linear(512 → 2048)

Loss:   SmoothL1(predicted_body_embedding, mean-centred body_embedding)
Target: body_embedding already stored in function_student_targets (float32)
        — uses same teacher / layer as Exp 1.1 so comparisons are valid.

Usage:
  # 8-GPU DDP (primary mode):
  torchrun --nproc-per-node=8 train_student_transformer.py

  # Single-GPU test:
  torchrun --nproc-per-node=1 train_student_transformer.py
...
RESULTS_FILE = os.path.join(os.path.dirname(__file__),
                             'docs', 'phase1_2_transformer_student.md')
...
def load_data(tokenizer) -> tuple[
    ...
    # Drop NaN embedding rows (from float16 overflow in teacher inference)
    valid   = ~np.isnan(body_raw).any(axis=1)
    n_nan   = (~valid).sum()
    if n_nan:
        print(f"  Dropped {n_nan} NaN body-embedding rows", flush=True)
...
def train(args, rank: int, world_size: int):
    ...
    if is_main:
        print(f"SWE-JEPA Experiment 1.2: Transformer Encoder Student")
        print(f"{'='*60}")
...
def write_results(res: dict):
    ...
    report = f"""# Experiment 1.2: Transformer Encoder Student
...
|--------|-------|----------|-------|
| Cosine sim (predicted vs actual body) | **{res['cos']:.4f}** | {res['bl_cos']:.4f} (sig→body) | {"↑" if res['cos'] > res['bl_cos'] else "↓"}{abs(res['cos']-res['bl_cos']):.4f} |
| Cosine sim (random body) | — | {res['rand_cos']:.4f} | Anisotropy baseline |
