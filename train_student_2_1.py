    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with capture_layer(frozen_qwen, TEACHER_LAYER) as captured:
            for batch in train_loader:
                ...
                # L2-normalise both before InfoNCE
                pred_n    = F.normalize(pred, dim=-1)
                body_n_t  = F.normalize(body_emb, dim=-1)

                # Cross-GPU gather: collect all ranks' body embeddings as negatives
        ...
        if is_main:
            ...
            with capture_layer(frozen_qwen, TEACHER_LAYER) as cap2, torch.no_grad():
                for batch in val_loader:
                    ...
                    hs2 = cap2[-1]
                    p = raw_pred(hs2, batch['attention_mask'].to(device))
...
|-----------|--------|---------|
| Val Rank@1 > 1% | > 1% | {"✅" if res['val_r1'] > 0.01 else "❌"} {res['val_r1']*100:.2f}% |
| Val Rank@10 > 5% | > 5% | {"✅" if res['val_r10'] > 0.05 else "❌"} {res['val_r10']*100:.2f}% |
| Test Rank@1 > 1% | > 1% | {"✅" if res['r1'] > 0.01 else "❌"} {res['r1']*100:.2f}% |
| Test Rank@10 > 5% | > 5% | {"✅" if res['r10'] > 0.05 else "❌"} {res['r10']*100:.2f}% |
"""
