"""
Experiment 4.2b SFT Baseline: Full fine-tune Qwen3-8B-base + InfoNCE.

Comparison vs SWE-JEPA (Exp 3.0) and the 3B SFT baseline (Exp 4.2a) at fixed compute
budget.  Same InfoNCE objective and 8B body embedding targets; only the student differs:

  JEPA:      frozen 8B teacher + 63M trainable student (sig-only input, no mismatch)
  SFT 3B:    3B model fine-tuned end-to-end (full function input, sig-only eval)
  SFT 8B:    8B model fine-tuned end-to-end (full function input, sig-only eval) ← this

Training input:  sig_text + '\\n' + body_text (up to 512 tokens)
Eval input:      sig_text only  (deliberate train-test mismatch — favours JEPA)
Target:          Qwen3-8B-base layer-18 body embeddings (same body_embs.npy as 3B SFT)
Projection:      Linear(4096 → 4096) — identity-shaped; learns alignment to teacher space
Quality log:     docs/sft_8b_compute_log.jsonl — {step, epoch, gpu_minutes, val_r10} per epoch

VRAM budget per card:
  Qwen3-8B-base bf16              ~16 GB
  Adafactor states (~2 bytes/p)   ~16 GB
  Activations (batch=4, 512 tok)  ~2 GB  (gradient checkpointing reduces activation peak)
  Headroom                        ~46 GB  → batch=4 should fit comfortably

Running:
    torchrun --nproc-per-node=8 train_sft_8b_baseline.py --body-emb-cache=body_embs.npy
"""
...
SFT_MODEL_PATH   = '/home/original_models/Qwen3-8B-base'
...
SFT_DIM          = 4096        # Qwen3-8B-base hidden_size
TEACHER_DIM      = 4096        # target dimension (same model family — no dim mismatch)
MAX_SIG_TOKENS   = 256
MAX_BODY_TOKENS  = 256
MAX_INPUT_TOKENS = 512         # sig + body concatenated

CKPT_PATH    = os.path.join(os.path.dirname(__file__), 'sft_8b_baseline_ckpt.pt')
LOG_PATH     = os.path.join(os.path.dirname(__file__), 'docs', 'sft_8b_compute_log.jsonl')
RESULTS_FILE = os.path.join(os.path.dirname(__file__), 'docs', 'phase4_2_sft_8b_results.md')
...
def evaluate_rank10(sft_model, proj_head, sig_ids_list, sig_masks_list,
    ...
    """
    Encode sig-only through the fine-tuned 8B at layer SFT_LAYER.
...
def train(args, rank, world_size, local_rank):
    ...
    if is_main:
        print(f"SWE-JEPA Experiment 4.2b: SFT Baseline (Qwen3-8B-base full fine-tune)")
        print(f"{'='*72}")
    ...
    # ── Tokeniser (8B) + data ─────────────────────────────────────────────────
    if is_main:
        print(f"\nLoading 8B tokeniser from {SFT_MODEL_PATH} …", flush=True)
    ...
    # ── Load body embs from cache (same targets as 3B SFT) ───────────────────
    # The body_embs.npy cache was created by the 3B SFT run with identical
    # precompute_body_embs logic (sig+body → 8B teacher layer 18).
    ...
    # ── Load trainable 8B model + projection head ─────────────────────────────
    if is_main:
        print(f"\nLoading Qwen3-8B-base for fine-tuning …", flush=True)
    sft_model = AutoModelForCausalLM.from_pretrained(
        SFT_MODEL_PATH, dtype=torch.bfloat16, trust_remote_code=True,
    ).to(device)

    # Gradient checkpointing always enabled for 8B to keep VRAM in budget
    sft_model.gradient_checkpointing_enable()
    if is_main:
        print("  Gradient checkpointing enabled (required for 8B on 80 GB).", flush=True)

    # Projection head: maps 8B dim → 8B target dim (identity-shaped, learns alignment)
    ...
    if is_main:
        print(f"  Trainable: {n_params:,} params "
              f"(8B model + proj head)", flush=True)
    ...
    # Adafactor: stores factored second moments (O(r+c) vs O(r×c)), reducing
    # optimizer state from ~96 GB (AdamW fp32) to ~16 GB for an 8B model.
    # relative_step=False + explicit lr avoids Adafactor's built-in LR schedule.
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with capture_layer(ddp_sft.module, SFT_LAYER) as captured:
            for batch in train_loader:
                ...
                pooled = torch.stack(pooled_list)   # [B, 4096]
                ...
                loss.backward()
                # Grad clipping kept for safety; Adafactor normalises by its own scale
...
def write_results(res):
    a = res['args']
    report = f"""# Experiment 4.2b SFT Baseline Results (8B)
...
**Optimizer**: Adafactor (lr={a['lr']}, relative_step=False) — reduces VRAM vs AdamW

## Architecture

| Component | Detail |
|-----------|--------|
| SFT model | Qwen3-8B-base ({res['n_params']:,} total params incl. proj) |
...
| Teacher (body targets only) | Qwen3-8B-base layer {TEACHER_LAYER} (loaded from body_embs.npy cache) |
...
| SFT 8B (this run) | {res['val_r10']*100:.2f}% | ~8B | sig+body → sig-only eval | {res['gpu_minutes']:.0f} |
...
def main():
    ap = argparse.ArgumentParser(description='SWE-JEPA Exp 4.2b: SFT 8B Baseline')
    ...
    ap.add_argument('--body-emb-cache', type=str,
                    default=None, help='Path to load/save precomputed body embeddings (numpy .npy).')
